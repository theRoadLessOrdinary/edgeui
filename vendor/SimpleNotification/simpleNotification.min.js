;
/**
 * @typedef {('top-left' | 'top-right' | 'bottom-left' | 'bottom-center' | 'bottom-right' | 'center-right')} Position
 */
...
class SimpleNotification {
    ...
    static deepAssign(target, ...objs) {
        for (let i = 0, max = objs.length; i < max; i++) {
            for (var k in objs[i]) {
                if (objs[i][k] != null && typeof objs[i][k] == 'object') target[k] = SimpleNotification.deepAssign(target[k] ? target[k] : {}, objs[i][k]); else target[k] = objs[i][k];
            }
        }
        return target;
    }
            ...
            if (typeof string[i] == 'string') {
                ...
                if ('attributes' in tagInfo) {
                    ...
                    for (let k = 0, max = keys.length; k < max; k++) {
                        let attributeValue = tagInfo.attributes[keys[k]].replace('$content', content).replace('$title', title);
